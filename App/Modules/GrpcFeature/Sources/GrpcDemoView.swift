    ...
    public func ask(_ sentence: String) {
        ...
        Task {
            do {
                ...
                // public Eliza demo) — Rust is the gRPC client here, not
                // Swift. Swift only ever sees a plain String, via the same
                // async-throws pattern as every other UniFFI export.
                let response = try await askEliza(sentence: sentence)
                reply = response
